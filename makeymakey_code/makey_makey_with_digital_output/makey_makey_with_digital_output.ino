 ...
 */
 /*   makey_makey_with_digital_output.ino
 ...
 Description:
   User output digital bits to send message protocol containing key pressed.
   Message protocol is to send number k:
     OUTPUT_1 = set
     Switch Output_2 to high k times
 Version Log:
   05/24/2014, Tiago Pimentel
     Changed updateOutLEDs() to also set output digital bits with message protocol.
*/
...
void updateOutLEDs()
{
  ...
  for (int i=0; i<NUM_INPUTS; i++)
  {
    if (inputs[i].pressed)
    {
      ...
      // Changes in Makey Makey code to send message protocol with number of the pressed key i
      // Set outputK to announce communication, and send i high bits using outputM
      if (!inputs[i].prevPressed)
      {
